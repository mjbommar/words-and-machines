...
absolute-value listings. Axeyum still lacks the typed cross-machine relation
that would turn those separate traces into one simulation result. This chapter
therefore presents the relation contract without giving the concrete examples
the stronger status of machine-established cross-ISA evidence.
...
The relation interface must expose the three entry states, the relation at
each named control point, one decoded segment step, the endpoint states, and
the first failed relation clause. Current Axeyum imports supply the machine
objects and steps but not the relation or failed-clause objects, so the book
does not print an executable relation call here.
\end{keyidea}
