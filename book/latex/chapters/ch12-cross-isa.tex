...
x86-64 decoder-and-step packages. It does not contain typed cross-machine
relations. This chapter therefore specifies the required relation without
presenting the absolute-value simulation as machine-established evidence. The
implemented routes show how decoded states and replay behave; the remaining
adapter and relation identify what must still be added rather than assumed.
...
The eventual interface must expose the three entry states, the relation at
each named control point, one decoded segment step, the endpoint states, and
the first failed relation clause. No current Axeyum import supplies those
objects, so the book does not print an executable call here.
\end{keyidea}
