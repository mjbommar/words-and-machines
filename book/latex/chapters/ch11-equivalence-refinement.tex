...
The first book adapter still needs \code{a0-state}, \code{a0-step}, and
\code{a0-run}, with code-byte provenance and negative controls. Represent an
A0 query with semantic-package digests, two decoded programs, word width,
precondition, behavior policy, observation, and a bound or termination
argument. Generate the negation of the exact reader statement. When a solver
finds a mismatch, decode a complete A0 initial state and replay both programs.
Reject the evidence if replay does not reproduce the mismatch.
